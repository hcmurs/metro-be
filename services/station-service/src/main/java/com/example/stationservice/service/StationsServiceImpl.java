...
public class StationsServiceImpl implements StationsService {
  ...
  @Autowired private StationRouteService stationRouteService;
  @Autowired private StationRouteRepository stationRouteRepository;
  ...
  public List<StationsResponse> getAllStations() {
    ...
    return list.stream().map(this::mapToStationsResponse).toList();
  }
    ...
    if (stationUpdate.getStationCode() != null) {
      if (!existingStation.getStationCode().equals(stationUpdate.getStationCode())
        ...
        throw new EntityNotFoundException(
            "Station code already exists: " + stationUpdate.getStationCode());
      }
  ...
  public void deleteStation(Long id) {
    ...
    List<StationRoute> stationRoutes =
        stationRouteRepository.findByStation_StationIdAndIsDeleted(id, false);
  ...
  //                .orElseThrow(() -> new EntityNotFoundException("Start station not found with id:
  // " + startStationId));
  //        Stations endStations = stationsRepository.findById(endStationId)
  //                .orElseThrow(() -> new EntityNotFoundException("End station not found with id: "
  // + endStationId));
  //        Stations thisStationObj = stationsRepository.findById(thisStation)
  //                .orElseThrow(() -> new EntityNotFoundException("This station not found with id:
  // " + thisStation));
  ...
  //        }
  //        List <Stations> list =
  // stationsRepository.findByRouteRouteIdOrderBySequenceOrder(routeId);
  //            return list.stream()
