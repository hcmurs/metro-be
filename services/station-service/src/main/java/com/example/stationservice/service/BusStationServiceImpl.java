...
public class BusStationServiceImpl implements BusStationsService {
  ...
  public BusStations getBusStationById(String id) {
    return busStationsRepository
        .findById(id)
        .orElseThrow(() -> new EntityNotFoundException("Bus Station not found with id: " + id));
  }
  ...
  public Response.BusStationResponse getBusStationRoutesStationId(String stationId) {
    BusStations busStation =
        busStationsRepository
            .findById(stationId)
            .orElseThrow(
                () -> new EntityNotFoundException("Bus Station not found with id: " + stationId));
    ...
    for (BusStationRoutes busStationRoute : busStationRoutes) {
      ...
      System.out.println(
          "Available route IDs: "
              + busRoutesRepository.findAll().stream().map(BusRoutes::getId).toList());
      Optional<BusRoutes> route = busRoutesRepository.findById(busStationRoute.getRouteId());
      if (route.isEmpty()) {
        throw new EntityNotFoundException(
            "Route not found with id: " + busStationRoute.getRouteId());
      }
      busRoutes.add(route.get());
    }
    return Response.BusStationResponse.builder()
        .id(busStation.getId())
