...
public class OrderController {
  ...
  @Operation(
      ...
      description = "Creates a new order for a ticket single.")
  public ApiResponse<OrderResponse> createOrderSingle(
      @RequestBody OrderTicketSingleRequest orderTicketSingleRequest,
  ...
  public ApiResponse<OrderResponse> createOrder(
      @RequestBody OrderTicketDaysRequest orderTicketDaysRequest,
      @RequestHeader("Authorization") String token) {
    // Logic to create an order
    String validatedToken = token.startsWith("Bearer ") ? token.substring(7) : token;
  ...
  @PreAuthorize("hasAuthority('ROLE_CUSTOMER')")
  @Operation(summary = "Get order by user ID", description = "Retrieves an order by the user's ID.")
  public ApiResponse<List<OrderResponse.OrderDetailResponse>> getOrderDetailByUserId(
      @RequestHeader("Authorization") String token) {
    String validatedToken = token.startsWith("Bearer ") ? token.substring(7) : token;
  ...
  @PreAuthorize("hasAuthority('ROLE_CUSTOMER')")
  @Operation(
      summary = "Update transaction for an order",
      description = "Updates the transaction details for an existing order by its ID.")
  ...
  @PreAuthorize("hasAuthority('ROLE_CUSTOMER')")
  @Operation(
      summary = "Get order details by status",
      description = "Retrieves order details by status for the authenticated user.")
  public ApiResponse<List<OrderResponse.OrderDetailResponse>> getOrderDetailByStatus(
      @RequestHeader("Authorization") String token, @RequestParam("status") String status) {
    String validatedToken = token.startsWith("Bearer ") ? token.substring(7) : token;
