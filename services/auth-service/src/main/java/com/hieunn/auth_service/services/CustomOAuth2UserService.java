...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
  ...
  private OAuth2User processOAuth2User(OAuth2UserRequest oAuth2UserRequest, OAuth2User oAuth2User) {
    ...
    SocialLoginUserRequest socialLoginRequest =
        new SocialLoginUserRequest(email, name, providerId, pictureUrl, authProvider);
    UserDto userDto = userServiceClient.processSocialLogin(socialLoginRequest).getData();

    return CustomOAuth2User.builder().userDto(userDto).attributes(attributes).build();
  }
}
