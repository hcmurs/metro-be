...
public class StatisticsServiceImpl implements StatisticsService {
  ...
  public void generateDailyStatistics(LocalDateTime start, LocalDateTime end) {
    try {
      List<TicketUsageLogDto> logs =
          ticketServiceClient.findAllTicketUsageLogsBetween(start, end).getData();
  ...
  private void generateStationUsageStatistics(List<TicketUsageLogDto> logs, LocalDateTime start) {
    try {
      final Map<Long, String> stationNameMap =
          findAllStation().stream()
      ...
      for (TicketUsageLogDto log : logs) {
        ...
        StationUsageStatistic stat =
            stationStats.computeIfAbsent(
                stationId,
                id ->
  ...
  private void generateTicketTypeStatistics(List<TicketUsageLogDto> logs, LocalDateTime start) {
    try {
      ...
      Map<String, Long> ticketTypeCounts =
          logs.stream()
              .map(
                  log -> {
                    String ticketCode = log.getTicketCode();
                    return ticketCodeToTypeMap.computeIfAbsent(
                        ticketCode,
                        code -> {
      ...
      List<TicketTypeStatistic> ticketTypeStats =
          ticketTypeCounts.entrySet().stream()
              .map(
                  entry ->
                      TicketTypeStatistic.builder()
  ...
  private void generatePeakHourStatistics(List<TicketUsageLogDto> logs, LocalDateTime start) {
    try {
      ...
      for (TicketUsageLogDto log : logs) {
        ...
        HourUsageStatistic stat =
            hourStats.computeIfAbsent(
                hour,
                h ->
