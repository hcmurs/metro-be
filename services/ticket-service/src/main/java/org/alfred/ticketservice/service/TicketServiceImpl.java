...
public class TicketServiceImpl implements TicketService, TicketCronJobService {
  ...
  @Autowired private TicketUsageLogRepository ticketUsageLogRepository;

  @Autowired private TicketRepository ticketRepository;

  @Autowired private FareMatrixRepository fareMatrixRepository;

  @Autowired private FareMatrixService fareMatrixService;
    ...
    try {
      ...
      TicketTypes ticketType =
          ticketTypeRepository
              ...
              .orElseThrow(
                  () ->
                      new EntityNotFoundException("Ticket type not found with id: " + ticket.id()));
      ...
      if (ticketType.getValidityDuration() < 0) {
        throw new IllegalArgumentException(
            "Ticket type must have a valid validity duration at least 0");
      }
      String token =
  ...
  public TicketResponse createTicketFare(TicketRequest.FareMatrix ticket) {
    ...
    FareMatrix fareMatrix =
        fareMatrixRepository
            .findById(ticket.id())
            .orElseThrow(
                () -> new EntityNotFoundException("Fare matrix not found with id: " + ticket.id()));
  ...
  public List<TicketResponse> getTicketByFareMatrixId(Long fareMatrixId) {
    FareMatrix fareMatrix =
        fareMatrixRepository
            .findById(fareMatrixId)
            .orElseThrow(
                () ->
                    new EntityNotFoundException("Fare matrix not found with id: " + fareMatrixId));
  ...
  public TicketResponse scanEntryTicket(TicketScanRequest ticketScanRequest) {
    ...
    try {
      ...
      TicketQrData ticketQrData =
          objectMapper.readValue(ticketScanRequest.qrCodeJsonData(), TicketQrData.class);
      ...
      if (ticket == null) {
        throw new EntityNotFoundException(
            "Ticket not found with code: " + ticketQrData.ticketCode());
      }

      if (ticket.getTicketType() == null) {
        throw new EntityNotFoundException(
            "Ticket type not found for ticket with code: " + ticketQrData.ticketCode());
      }
      ...
      if (ticket.getTicketType().getValidityDuration() == 0) {
        if (ticket.getFareMatrix() == null) {
          throw new EntityNotFoundException(
              "Fare matrix not found for ticket with code: " + ticketQrData.ticketCode());
        }
        if (ticket.getFareMatrix().getStartStationId() == null) {
          throw new EntityNotFoundException(
              "Start station not found for fare matrix of ticket with code: "
                  + ticketQrData.ticketCode());
        }
        if (!ticket.getFareMatrix().isActive()) {
          throw new EntityNotFoundException(
              "Fare matrix is not active for ticket with code: " + ticketQrData.ticketCode());
        }
        ...
        saveTicketUsageLog(
            ticket, UsageTypes.ENTRY, currentScanTime, ticketScanRequest.stationId());
      } else {
        if (ticket.getStatus() == TicketStatus.NOT_USED) {
          ticket.setValidUntil(
              currentScanTime.plusDays(ticket.getTicketType().getValidityDuration()));
          ticket.setStatus(TicketStatus.USED);
          ticket.setInTrip(true);
          saveTicketUsageLog(
              ticket, UsageTypes.ENTRY, currentScanTime, ticketScanRequest.stationId());
      ...
      if (ticket.getTicketType().getValidityDuration() == 0) {
        if (ticket.getStatus() == TicketStatus.USED) {
          ...
          if (ticket.getFareMatrix() == null || ticket.getFareMatrix().getEndStationId() == null) {
            throw new EntityNotFoundException(
                "Fare matrix or end station not found for ticket with code: "
                    + ticketQrData.ticketCode());
          }

          if (!fareMatrixService.isStationInFareMatrix(
              ticketScanRequest.stationId(), ticket.getFareMatrix().getFareMatrixId())) {
  ...
  public List<TicketResponse> getTicketsByTicketType(Long ticketTypeId) {
    ...
    TicketTypes ticketType =
        ticketTypeRepository
            ...
            .orElseThrow(
                () ->
                    new EntityNotFoundException("Ticket type not found with id: " + ticketTypeId));
  ...
  private TicketResponse mapToResponse(Tickets ticket) {
    return TicketResponse.builder()
        ...
        .fareMatrixId(
            ticket.getFareMatrix() == null ? null : ticket.getFareMatrix().getFareMatrixId())
  ...
  private String generateSignature(TicketQrData ticketQrData) {
    try {
      ...
      data.append(ticketQrData.ticketTypeName() != null ? ticketQrData.ticketTypeName() : "")
          .append(
              ticketQrData.ticketId() != null
                  ? ticketQrData.ticketId()
                  : "") // .append(ticketQrData.name() != null ? ticketQrData.name() : "")
  ...
  private void saveTicketUsageLog(
      Tickets ticket, UsageTypes usageType, LocalDateTime usageTime, Long stationId) {
    TicketUsageLogs ticketUsageLog =
        TicketUsageLogs.builder()
            .ticket(ticket)
            .usageType(usageType)
            .usageTime(usageTime)
            .stationId(stationId)
  ...
  private String generateQrContent(Tickets ticket) {
    try {
      ...
      TicketQrData qrDataWithoutSignature =
          TicketQrData.builder()
  ...
  public void expireTickets() {
    ...
    statusesToExpire.forEach(
        status -> {
          List<Tickets> expiredTickets =
              ticketRepository.findByStatusAndValidUntilBefore(status, now);
