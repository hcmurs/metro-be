...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleInvalidJson(HttpMessageNotReadableException ex) {
    ApiResponse<Void> response =
        ApiResponse.error(HttpStatus.BAD_REQUEST.value(), "Invalid request format");
    return ResponseEntity.ok().body(response);
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleIllegalArgument(IllegalArgumentException ex) {
    ApiResponse<Void> response =
        ApiResponse.error(HttpStatus.BAD_REQUEST.value(), "Lỗi rồi" + ex.getMessage());
    return ResponseEntity.ok().body(response);
  }
  ...
  @ExceptionHandler({
    ...
    TicketProcessingException.class,
    QrProcessingException.class
  })
  @ResponseStatus(HttpStatus.OK)
  public ResponseEntity<ApiResponse<Void>> handleExistEntity(RuntimeException ex) {
    ApiResponse<Void> response = ApiResponse.error(HttpStatus.BAD_REQUEST.value(), ex.getMessage());
    return ResponseEntity.ok().body(response);
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleGeneralException(Exception ex) {
    ...
    ApiResponse<Void> response =
        ApiResponse.error(HttpStatus.INTERNAL_SERVER_ERROR.value(), "Unexpected error occurred");
    return ResponseEntity.ok().body(response);
  }
