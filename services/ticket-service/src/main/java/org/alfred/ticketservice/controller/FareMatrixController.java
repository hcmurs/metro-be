...
public class FareMatrixController {
  ...
  public ResponseEntity<ApiResponse<List<FareMatrixResponse>>> getAllFareMatrices() {
    ...
    return ResponseEntity.ok(
        ApiResponse.success(fareMatrices, "Fare matrices retrieved successfully"));
  }

  @PostMapping("/create")
  @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_STAFF')")
  public ResponseEntity<ApiResponse<FareMatrixResponse>> createFareMatrix(
      @Valid @RequestBody FareMatrixRequest request) {
    log.info(
        "Request to create fare matrix between stations: {} and {}",
  ...
  @PreAuthorize("hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_STAFF')")
  public ResponseEntity<ApiResponse<FareMatrixResponse>> updateFareMatrix(
      @Valid @RequestBody FareMatrixRequest request, @PathVariable Long fareMatrixId) {
    FareMatrixResponse updatedFareMatrix =
        fareMatrixService.updateFareMatrix(request, fareMatrixId);
    return ResponseEntity.ok(
        ApiResponse.success(updatedFareMatrix, "Fare matrix updated successfully"));
  }
  ...
  public ResponseEntity<ApiResponse<List<FareMatrixResponse>>> getFareMatricesByStartStation(
    ...
    List<FareMatrixResponse> fareMatrices =
        fareMatrixService.getFareMatricesByStartStationId(stationId);
    return ResponseEntity.ok(
        ApiResponse.success(fareMatrices, "Fare matrices retrieved successfully"));
  }
  ...
  @PreAuthorize(
      "hasAuthority('ROLE_CUSTOMER') or hasAuthority('ROLE_ADMIN') or hasAuthority('ROLE_STAFF')")
  public ResponseEntity<ApiResponse<Boolean>> validateStationInFareMatrix(
      @PathVariable Long fareMatrixId, @PathVariable Long stationId) {
    log.info(
        "Request to validate if station ID: {} is in fare matrix ID: {}", stationId, fareMatrixId);
    boolean isValid = fareMatrixService.isStationInFareMatrix(stationId, fareMatrixId);
    return ResponseEntity.ok(
        ApiResponse.success(
            isValid,
            isValid
                ? "Station is valid for this fare matrix"
                : "Station is not valid for this fare matrix"));
  }
  ...
  public ResponseEntity<ApiResponse<FareMatrixResponse>> updateStatusFare(
    ...
    return ResponseEntity.ok(
        ApiResponse.success(updatedFareMatrix, "Fare matrix status updated successfully"));
  }
}
