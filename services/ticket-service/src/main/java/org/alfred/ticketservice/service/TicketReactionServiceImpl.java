...
public class TicketReactionServiceImpl implements TicketReactionService {
  ...
  public void handleDeleteStation(StationEvent event) {
    try {
      ApiResponse<StationRouteResponse> stationRouteResponse =
          stationClient.getStationRouteById(event.stationRouteId());
      ...
      List<FareMatrixResponse> fareMatrixResponses =
          fareMatrixService.getFareMatricesByEndStationIdorStartId(stationRoute.id());
      ...
      boolean shouldActivate =
          !(stationRoute.isDeleted() || stationRoute.status() != StationStatus.active);
      ...
      for (FareMatrixResponse fareMatrixResponse : fareMatrixResponses) {
        Optional<FareMatrix> fareMatrixOpt =
            fareMatrixRepository.findById(fareMatrixResponse.fareMatrixId());
      ...
      if (!fareMatricesToUpdate.isEmpty()) {
        ...
        log.info(
            "Updated {} fare matrices for station route: {}",
            fareMatricesToUpdate.size(),
            event.stationRouteId());
      }

    } catch (Exception e) {
      log.error(
          "Error handling delete station event for station route: {}", event.stationRouteId(), e);
  ...
  public void handleAddStation(StationEvent event) {
    try {
      ...
      if (stationRoute.isDeleted() || stationRoute.status() != StationStatus.active) {
        log.info(
            "Station route is not active, skipping fare matrix creation for: {}",
            event.stationRouteId());
        return;
      }
      ...
      List<StationRouteResponse> allStationsInRoute =
          stationClient.getStationRoutesByRouteId(stationRoute.RouteId()).getData();
      ...
      for (StationRouteResponse otherStation : allStationsInRoute) {
        ...
        FareMatrixRequest fareMatrixFrom =
            FareMatrixRequest.builder()
                .name(
                    stationRoute.stationsResponse().name()
                        + "-"
                        + otherStation.stationsResponse().name())
        ...
        FareMatrixRequest fareMatrixTo =
            FareMatrixRequest.builder()
                .name(
                    otherStation.stationsResponse().name()
                        + "-"
      ...
      if (!fareMatricesToCreate.isEmpty()) {
        ...
        log.info(
            "Created {} fare matrices for new station: {} in route: {}",
            fareMatricesToCreate.size(),
            newStationId,
            stationRoute.RouteId());
      } else {
        log.info(
            "No fare matrices created for station: {} in route: {}",
            newStationId,
            stationRoute.RouteId());
      }
      ...
      log.error(
          "Error handling add station event for station route: {}", event.stationRouteId(), e);
