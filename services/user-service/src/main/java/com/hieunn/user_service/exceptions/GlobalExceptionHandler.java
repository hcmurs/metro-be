...
public class GlobalExceptionHandler {
  ...
  public ResponseEntity<ApiResponse<Void>> handleNullPointerException(NullPointerException e) {
    return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.error(500, e.getMessage()));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleMissingRequestHeaderException(
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            ApiResponse.error(
                ErrorMessage.ALREADY_LOGOUT.getStatus(), ErrorMessage.ALREADY_LOGOUT.getMessage()));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleDataIntegrityViolationException(
    ...
    return ResponseEntity.status(HttpStatus.OK).body(ApiResponse.error(400, message));
  }
  ...
  public ResponseEntity<ApiResponse<Void>> handleAuthorizationDeniedException(
    ...
    return ResponseEntity.status(HttpStatus.OK)
        .body(
            ApiResponse.error(
                ErrorMessage.UNAUTHORIZED.getStatus(), ErrorMessage.UNAUTHORIZED.getMessage()));
  }
