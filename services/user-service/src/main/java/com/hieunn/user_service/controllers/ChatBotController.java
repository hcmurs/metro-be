...
public class ChatBotController {
    ...
    public ResponseEntity<ApiResponse<ChatResponse>> chat(@RequestBody ChatRequest request) {
        try {
            ...
            return ResponseEntity.ok(ApiResponse.success(chatResponse, "Chat response generated successfully"));
        } catch (Exception e) {
            log.error("Error processing chat request: ", e);

            ChatResponse errorResponse = new ChatResponse(request.getSessionId(), "Sorry, max length token usage for this conversation. Please try again.");
    ...
    public ResponseEntity<ApiResponse<String>> health() {
        return ResponseEntity.ok(ApiResponse.success("ChatBot service is running", "Health check successful"));
    }
}
