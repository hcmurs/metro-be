...
public class UserServiceImpl implements UserService, UserDetailsService {
  ...
  public UserDto processSocialLogin(SocialLoginRequest socialLoginRequest) {
    ...
    Optional<User> userByProviderId =
        findUserByProviderId(provider, socialLoginRequest.getProviderId());
  ...
  private UserDto updateExistingUser(User existingUser, SocialLoginRequest socialLoginRequest) {
    ...
    if (existingUser.getName() == null
        || !existingUser.getName().equals(socialLoginRequest.getName())) {
      existingUser.setName(socialLoginRequest.getName());
      changed = true;
    }

    if (existingUser.getPictureUrl() == null
        || !existingUser.getPictureUrl().equals(socialLoginRequest.getPictureUrl())) {
    ...
    if (socialLoginRequest.getAuthProvider() == AuthProvider.GOOGLE
        && (existingUser.getGoogleId() == null
            || !existingUser.getGoogleId().equals(socialLoginRequest.getProviderId()))) {
    ...
    if (socialLoginRequest.getAuthProvider() == AuthProvider.FACEBOOK
        && (existingUser.getFacebookId() == null
            || !existingUser.getFacebookId().equals(socialLoginRequest.getProviderId()))) {
  ...
  public void resetPassword(String email, String newPassword) {
    ...
    if (userByEmail.isEmpty()) {
      throw new CustomException(
          ErrorMessage.USER_NOT_FOUND.getStatus(), ErrorMessage.USER_NOT_FOUND.getMessage());
    }
  ...
  public UserDto processLocalLogin(LocalLoginRequest localLoginRequest) {
    Optional<User> userByUsername =
        userRepository.findByUsername(localLoginRequest.getUsernameOrEmail());
